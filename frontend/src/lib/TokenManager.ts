 ...
 * 
 * Single source of truth for token storage - prevents conflicts
...
class TokenManager {
  ...
  private refreshTokenListeners: Set<TokenListener> = new Set();
   ...
   * Returns unsubscribe function
   */
  public subscribeRefreshToken(listener: TokenListener): () => void {
  ...
  private handleStorageChange = (event: StorageEvent): void => {
    if (event.key === this.storageKey) {
      this.notifyAccessTokenListeners(event.newValue);
    } else if (event.key === this.refreshStorageKey) {
      this.notifyRefreshTokenListeners(event.newValue);
    }
  };
