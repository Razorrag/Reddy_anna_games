...
import { Search, Filter, Check, X, Eye, Download, RefreshCw, Clock, CheckCircle2, XCircle } from 'lucide-react';
...
import { useApproveDepositMutation } from '@/hooks/mutations/useApproveDepositMutation';
import { useRejectDepositMutation } from '@/hooks/mutations/useRejectDepositMutation';
...
export default function DepositRequests() {
  ...
  const { data, isLoading, refetch } = useDepositRequestsQuery(filters);
  const approveMutation = useApproveDepositMutation();
  ...
  const handleExport = () => {
    const csv = [
      ['ID', 'User', 'Phone', 'Amount', 'Status', 'Created', 'Processed'].join(','),
      ...requests.map((r: any) => [
        r.id,
    ...
    a.href = url;
    a.download = `deposits-${Date.now()}.csv`;
    a.click();
      ...
      >
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label: 'Approved', value: data?.stats?.approved || 0, color: 'green' },
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl overflow-hidden">
          {isLoading ? (
            ...
            <div className="text-center p-12">
              <p className="text-gray-400 mb-2">No deposit requests found</p>
              ...
              <table className="w-full">
                <thead className="bg-white/5 border-b border-white/10">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-400">Screenshot</th>
          ...
          >
            ...
            >
              <h3 className="text-xl font-bold text-white mb-4">
                {actionDialog.action === 'approve' ? 'Approve Deposit' : 'Reject Deposit'}
              </h3>
